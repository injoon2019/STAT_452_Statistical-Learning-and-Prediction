...
data = data.in %>%
  transmute(quality = case_when(quality < 6 ~ "low",
    quality == 6 ~ "med",
    quality > 6 ~ "high"),
    sugar = residual.sugar, density = density, pH = pH,
    sulphates = sulphates, chlor = chlorides,
    alcohol = alcohol) %>%
  filter(sulphates < 1.4, pH < 3.85, chlor <= 0.3)
